...
exports.handleParticipation = function (cmd, args, msg) {
    ...
    if (msg.channel.parentID === utils.getRaidCatId(server)) {
        ...
        getParticipationMsg(msg.channel).then((participationMsg) => {
            ...
            let participantData = getParticipantData(messContent); // todo delete
            const userMessageData = getMsgData(args, msg); // todo delete

            raidMessage.checkTime(msg.channel.name).then(() => {
                sessionsData.updateSessions(raidMessage, cmd);
            }).catch(err => console.log(err));

            // vérification du format de l'heure
            checkTime(userMessageData.time, msg.channel.name).then((time) => { // todo mettre dans RaidMessage
